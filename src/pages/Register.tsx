...
const Register: React.FC = () => {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      const result = await client.models.Firm.create(payload);
      console.log('Firm created:', result);
    } catch (err) {
  ...
  const PageContainer = useRef(styled.div`
    display: flex;
  ...
  const LogoContainer = useRef(styled.div`
    margin-bottom: 10px;
  `).current;

  const Logo = useRef(styled.img`
    width: clamp(56px, 10vw, 88px);
    height: clamp(56px, 10vw, 88px);
  `).current;

  const Title = useRef(styled.h1`
    margin: 0;
    color: #333;
  ...
  const Version = useRef(styled.p`
    margin: 6px 0 24px;
    color: #6c757d;
    font-size: clamp(12px, 1.5vw, 14px);
  `).current;

  const FormGroup = useRef(styled.div`
    margin-bottom: 20px;
    text-align: left;
  `).current;

  const Label = useRef(styled.label`
    display: block;
    margin-bottom: 6px;
    font-size: 14px;
    color: #333;
  ...
  const Input = useRef(styled.input`
    width: 100%;
  ...
  const Select = useRef(styled.select`
    width: 100%;
    padding: clamp(10px, 1.4vw, 14px) clamp(12px, 1.6vw, 16px);
    font-size: clamp(14px, 1.8vw, 16px);
    font-family: inherit;
  ...
  const Col = useRef(styled.div`
    flex: 1;
  `).current;

  

  const BrandHeader = useRef(styled.div`
    text-align: left;
    margin-bottom: 8px;
  `).current;

  const BackButton = useRef(styled.button`
    position: absolute;
  ...
  const FormColumns = useRef(styled.div`
    display: grid;
    grid-template-columns: 1fr;
    gap: 16px;
  `).current;

  const FormCol = useRef(styled.div`
    display: flex;
    flex-direction: column;
    gap: 16px;
  `).current;

  const Button = useRef(styled.button`
    width: 100%;
  ...
  const Primary = useRef(styled(Button)`
